...
@Component({
  selector: 'app-eliminar-empleado-masivo',
  templateUrl: './eliminar-empleado-masivo.component.html',
  styleUrls: ['./eliminar-empleado-masivo.component.css']
})
export class EliminarEmpleadoMasivoComponent implements OnInit {
  constructor(
  ...
  onSaveEmpelados(formCarga): void {
    ...
    if (this.temporalJson.length > 0) {
      this.dataApi
        .Post('/Usuarios/EliminarEmpMasivo', this.excelEmpleados)
        .subscribe(
          (result) => {
            if (result.exito == 1) {
