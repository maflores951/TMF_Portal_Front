import { UpdateEmpresaEmpleado } from './../../../../models/updateEmpresaEmpleado';
...
@Component({
  selector: 'app-actualizar-empresa-masivo',
  templateUrl: './actualizar-empresa-masivo.component.html',
  styleUrls: ['./actualizar-empresa-masivo.component.css']
})
export class ActualizarEmpresaMasivoComponent implements OnInit {
  constructor(
  ...
  private excelEmpleados: UpdateEmpresaEmpleado[] = [];
      ...
      if (encabezado.length == 3) {
        if (encabezado[0] == 'EmpresaId') {
          if (index > 0) {
            if (element.length == 3){
              var empleado = {
                empresaIdOld: element[0],
                empresaIdNew: element[1],
                empleadoNoEmp: element[2],
              };
  ...
  onSaveEmpelados(formCarga): void {
    ...
    if (this.temporalJson.length > 0) {
      this.dataApi
        .Post('/Usuarios/ActualizarEmpEmpresaMasivo', this.excelEmpleados)
        .subscribe(
          (result) => {
            if (result.exito == 1) {
