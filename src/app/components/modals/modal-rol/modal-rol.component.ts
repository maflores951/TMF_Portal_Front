...
export class ModalRolComponent implements OnInit {
  ...
  get RolNombre() {
    return this.UsuarioForm.get('RolNombre');
  }
  ...
  user_validation_messages = {
    RolNombre: [
      ...
      },
      {
        type: 'minlength',
        message:
          'El nombre del tipo de usuario debe de contener mínimo 4 caracteres',
      },
    ],
  };
  ...
  onSaveUserType(formUserType): void {
    ...
    if (this.UsuarioForm.valid) {
      if (this.UsuarioForm.value.RolId == null) {
        ...
        this.dataApi.Post('/Roles', this.UsuarioForm.value).subscribe(
          (result) => {
