...
export class CifradoDatosService {
  ...
  desEncrypt(conversion: string): string {
    let keyAndIv = CryptoJS.PBKDF2(this.key, this.salt, { keySize: 256/32 + 128/32, iterations: 1000, hasher: CryptoJS.algo.SHA1 }); // so PBKDF2 in CryptoJS is direct in that it
  ...
  encrypt(conversion: string): string {
    ...
    // console.log(conversion)
     var encTarjeta = CryptoJS.AES.encrypt(CryptoJS.enc.Utf16LE.parse(conversion),key,
      {
        keySize: 128 / 8,
        iv: iv,
        mode: CryptoJS.mode.CBC,
